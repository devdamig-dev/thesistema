...
export function RubroClient({ initialIndustry }: Props) {
  const [industry, setIndustry] = useState<IndustryKey>(initialIndustry);
  const [savedIndustry, setSavedIndustry] = useState<IndustryKey>(initialIndustry);
          ...
          {INDUSTRIES.map((i) => {
            ...
            return (
              ...
              >
                ...
                <div className="text-[11px] leading-snug text-ink-muted">{i.tagline}</div>
              </button>
            );
          })}
        </div>
        ...
        footer={
          <>
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setIndustry(savedIndustry)}
              disabled={pending || industry === savedIndustry}
            >
              Restablecer
            </Button>
            <Button
