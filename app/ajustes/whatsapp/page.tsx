  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-[1fr_auto]">
          <div className="flex items-start gap-3 rounded-xl border border-success-500/25 bg-success-500/[0.06] p-4">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-xs text-ink-muted">Última sincronización hace 2 min · 8 mensajes hoy</p>
              <div className="mt-3 flex gap-2">
                <Button variant="ghost" size="sm" onClick={() => toast({ tone: "ai", title: "Resincronizando…" })}>
                  <RefreshCw className="h-3.5 w-3.5" /> Resincronizar
                </Button>
                <Button variant="ghost" size="sm" onClick={() => toast({ tone: "warn", title: "Conexión pausada" })}>
                  Pausar conexión
                </Button>
              </div>
            </div>
          ...
          <div className="grid h-full place-items-center rounded-xl border border-line bg-bg-subtle/40 p-4">
            <div className="text-center">
              ...
              <Button variant="primary" size="sm" className="mt-3" onClick={() => toast(ToastPresets.comingSoon("Reescanear QR"))}>
                Reescanear QR
              </Button>
            </div>
          </div>
      ...
      <SettingsCard
        ...
        footer={<Button variant="primary" size="sm" onClick={() => toast(ToastPresets.comingSoon("Invitar miembro"))}><Users className="h-3.5 w-3.5" /> Invitar miembro</Button>}
      >
        <ul className="space-y-2">
          {DEMO_AUTHORIZED.map((p) => (
            <li key={p.phone} className="flex items-center gap-3 rounded-xl border border-line bg-bg-subtle/40 p-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2"><span className="text-sm font-semibold text-ink">{p.name}</span><Badge tone="default">{p.role}</Badge></div>
                <div className="flex items-center gap-1 text-xs text-ink-muted"><Phone className="h-3 w-3" />{p.phone}</div>
              </div>
              <Badge tone={p.active ? "success" : "default"}>{p.active ? "Activo" : "Pausado"}</Badge>
              <button onClick={() => toast({ tone: "neutral", title: `${p.name} fue removido` })} className="rounded-md p-1.5 text-ink-subtle hover:text-danger-400" aria-label="Quitar">
      ...
      <SettingsCard
        ...
        footer={<Button variant="primary" size="sm" onClick={() => toast(ToastPresets.settingsSaved())}><Check className="h-3.5 w-3.5" /> Guardar</Button>}
      >
        <SettingRow label="Saludo automático" hint="Cuando arranca una conversación"><TextField defaultValue="¡Hola! Soy el copiloto de La Birra. ¿Qué movimiento querés cargar?" /></SettingRow>
        <SettingRow label="Pedido de dato faltante"><TextField defaultValue="¿Cómo lo pagaron? Es el dato que me falta para registrarlo." /></SettingRow>
        <SettingRow label="Pedir aclaración automáticamente" hint="Si la confianza es < 70%"><Toggle defaultChecked /></SettingRow>
        <SettingRow label="Notificar al socio si la IA duda" hint="Por WhatsApp y dashboard"><Toggle defaultChecked /></SettingRow>
      </SettingsCard>
    </div>
  );
}
