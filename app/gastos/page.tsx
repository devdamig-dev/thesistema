...
import { getExpensesPageDataAction, type ExpenseRow, type ExpensesPageData } from "@/app/actions/expenses-page";
...
export default function GastosPage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    void getExpensesPageDataAction()
      .then((res) => {
        if (cancelled) return;
        if (!res.ok) {
      ...
      .catch(() => {
        if (!cancelled) setLoadError("No pudimos cargar los gastos reales.");
      })
      .finally(() => {
    ...
    <div className="space-y-8">
      <SectionHeader
        ...
        description={IS_DATABASE
          ? "Costos estructurales persistidos y compras reales del mes. El punto de equilibrio sólo se calcula cuando existe un margen guardado."
          : "Costos estructurales del mes y cuánto tenés que facturar diariamente para cubrirlos."}
        actions={
          <Button size="sm" variant="primary" onClick={() => toast(ToastPresets.comingSoon("Alta de gasto fijo"))}>
      ...
      {IS_DATABASE && loadError && (
        <div className="rounded-2xl border border-warn-500/30 bg-warn-500/[0.06] p-5">
          <div className="text-sm font-semibold text-ink">No pudimos leer el estado real de Gastos</div>
          <p className="mt-1 text-xs text-ink-muted">{loadError} No mostramos datos demo ni ceros como si fueran información confirmada.</p>
        </div>
      )}

      {IS_DATABASE && loading ? (
        <div className="rounded-2xl border border-line p-8 text-center text-sm text-ink-muted">
          <Loader2 className="mx-auto mb-2 h-5 w-5 animate-spin" /> Cargando gastos reales…
        </div>
      ) : loadError ? null : (
        <>
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            <KpiCard label="Costos fijos" value={formatARS(totalFijos, { compact: true })} tone="brand" hint="Registros de gastos" />
            <KpiCard label="Costos variables (mes)" value={formatARS(totalVariables, { compact: true })} hint="Compras persistidas del mes" />
            ...
            <KpiCard label="Punto de equilibrio mes" value={ventasMensuales == null ? "—" : formatARS(ventasMensuales, { compact: true })} icon={<Target />} tone="success" hint={ventasMensuales == null ? "Requiere margen real" : `Escenario: ${SCENARIO_LABEL[scenario].label}`} />
          </div>
          ...
          <Card>
            ...
            {expenses.length === 0 ? (
              <CardContent>
                <div className="rounded-xl border border-dashed border-line px-4 py-8 text-center text-sm text-ink-muted">Todavía no hay gastos registrados.</div>
              </CardContent>
            ) : (
                  ...
                  <tbody>
                    {expenses.map((expense) => (
                      <tr key={expense.id} className="border-b border-line/60 last:border-0 hover:bg-bg-subtle">
                        ...
                        <td className="px-5 py-3"><Badge tone={statusTone(expense.estado)}>{expense.estado}</Badge></td>
          ...
          <Card>
            ...
            <CardContent className="space-y-5">
              {!hasMargin ? (
                <div className="rounded-xl border border-dashed border-line px-4 py-8 text-center text-sm text-ink-muted">Todavía no hay un balance con margen bruto. No calculamos un punto de equilibrio ficticio.</div>
              ) : (
                <>
