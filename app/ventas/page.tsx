...
import { getSalesPageDataAction, type SalesPageData } from "@/app/actions/sales-page";
...
export default function VentasPage() {
  ...
  useEffect(() => {
    ...
    void getSalesPageDataAction()
      ...
      .catch(() => {
        if (!cancelled) setLoadError("No pudimos cargar las ventas reales.");
      })
      .finally(() => {
      ...
      <div className="space-y-8">
        <SectionHeader
          ...
          description={IS_DATABASE
            ? "Los indicadores se calculan con los movimientos de venta persistidos en Supabase."
        ...
        {IS_DATABASE && loadError && (
          <div className="rounded-2xl border border-warn-500/30 bg-warn-500/[0.06] p-5">
            <div className="text-sm font-semibold text-ink">No pudimos leer el estado real de Ventas</div>
        ...
        {IS_DATABASE && loading ? (
          <div className="rounded-2xl border border-line p-8 text-center text-sm text-ink-muted">
            <Loader2 className="mx-auto mb-2 h-5 w-5 animate-spin" /> Cargando ventas reales…
          </div>
        ) : loadError ? null : (
          <>
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-5">
              <Card className="lg:col-span-3">
                <CardHeader><div><CardTitle>Ritmo de ventas</CardTitle><p className="mt-0.5 text-xs text-ink-muted">Últimos 11 días con movimientos</p></div></CardHeader>
                <CardContent>{salesByDay.length === 0 ? <EmptyState text="Todavía no hay ventas para graficar." /> : <SalesAreaChart data={salesByDay} />}</CardContent>
              </Card>
              <Card className="lg:col-span-2">
                ...
                <CardContent>{salesByChannel.length === 0 ? <EmptyState text="Todavía no hay ventas por canal." /> : <ChannelBar data={salesByChannel.map((c) => ({ canal: c.canal, total: c.total }))} />}</CardContent>
              </Card>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
              <Card className="lg:col-span-2">
                <CardHeader><CardTitle>Ventas diarias</CardTitle><p className="text-xs text-ink-muted">Últimos 7 días con movimientos</p></CardHeader>
                {dailySalesTable.length === 0 ? <CardContent><EmptyState text="Todavía no hay ventas diarias registradas." /></CardContent> : (
