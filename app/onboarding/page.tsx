...
async function getInitialState(): Promise<OnboardingInitialState> {
  if (!isDatabaseMode()) return DEFAULT_ONBOARDING_STATE;

  const supabase = createSupabaseServerClient();
  if (!supabase) return DEFAULT_ONBOARDING_STATE;
  const db = supabase as any;

  const { data: authData, error: authError } = await db.auth.getUser();
  const userId = authData?.user?.id as string | undefined;
  if (authError || !userId) return DEFAULT_ONBOARDING_STATE;

  const membershipsRes = await db
    .from("business_members")
    .select("business_id, role")
    .eq("user_id", userId);
  if (membershipsRes.error) return DEFAULT_ONBOARDING_STATE;
  ...
  if (ownerBusinessIds.length === 0) return DEFAULT_ONBOARDING_STATE;
  ...
  if (businessesRes.error) return DEFAULT_ONBOARDING_STATE;
  ...
  if (incomplete.length !== 1) return DEFAULT_ONBOARDING_STATE;
