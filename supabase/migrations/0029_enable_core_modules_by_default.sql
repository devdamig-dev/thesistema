-- Production module availability must not be reduced to the industry's
-- recommendations. Suggestions are highlights; implemented core modules remain
-- available unless a business explicitly disables one.

-- Backfill existing tenants with every module that is currently implemented in
-- the application sidebar/route guards. Existing rows (including an explicit
-- enabled=false) are preserved.
insert into public.business_modules (business_id, module_key, enabled, suggested)
select
  b.id,
  core.module_key,
  true,
...
cross join unnest(array[
  ...
  'employees',
  'customers'
]::public.module_key[]) as core(module_key)
on conflict (business_id, module_key) do nothing;

-- Future first-business onboarding must seed the implemented core catalog and
-- use p_modules only to mark the industry recommendations as suggested.
...
declare
  ...
  v_core_modules public.module_key[] := array[
    'dashboard',
    'inbox_ai',
    'reports_ai',
    'marketing_ai',
...
begin
  ...
  if v_membership_count > 0 then
    ...
    select v_business_id, module_key, true, false
    from unnest(v_core_modules) module_key
    on conflict (business_id, module_key) do nothing;
